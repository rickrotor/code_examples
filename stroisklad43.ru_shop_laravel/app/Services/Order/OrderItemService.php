...
class OrderItemService
{
    ...
    public function generateOrderItems(array $productsList, Order $updatedOrder): Collection
    {
        ...
        foreach ($productsList as $productId => $quantity) {
            ...
            $productName = $this->getProductName($productId, $existingItems);
     ...
     * @return |null
     */
    public function getProductName(int $productId, Collection $existingItems)
    {
        $product = $this->productRepository->getById($productId);
        $existingItemsName = $existingItems->where("product_id", $productId)->first()->name ?? null;
        return $existingItemsName ?? $product->name;
    }
}